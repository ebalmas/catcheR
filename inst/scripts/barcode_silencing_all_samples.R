...
if(empty == F){
  ...
  matrix = read.table(paste(dir, "Results_1/silencing_matrix_1.csv", sep=""), sep = ",", header = TRUE, row.names = 1)
  names(matrix) = paste(names(matrix), 1, sep = "_")
  
  if (samples > 1) {
    # Loop through the remaining files and bind them
    for (s in 2:samples) {
      m = read.table(paste(dir, "Results_", s, "/silencing_matrix_", s, ".csv", sep=""), sep = ",", header = TRUE, row.names = 1)
  ...
  write.csv(matrix, paste(dir,"/silencing_matrix_all_samples.csv",sep=""))
  saveRDS(matrix, file = paste(dir,"/silencing_matrix_all_samples.rds",sep=""))
  ...
  saveRDS(matrix, file = paste(dir,"/silencing_matrix_complete_all_samples.rds",sep=""))
