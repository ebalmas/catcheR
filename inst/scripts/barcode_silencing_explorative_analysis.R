...
#plot zoom 
p <- p + coord_cartesian(xlim = c(0,as.numeric(x)) ,ylim = c(0, as.numeric(y))) 

ggsave(filename = paste0(dir, "/UMIxUCI_", x, "_", y, ".jpg"),
       plot = p,
       width = 7, height = 9)
pdf(paste(dir,"/UMIxUCI_", y, "_", y, ".pdf",sep=""))
p
dev.off()
...
#Save complete table
head(tr_complete_table_fin)
write.csv(tr_complete_table_fin, paste(dir,"/complete_table_fin_", sample, ".csv",sep=""))
saveRDS(tr_complete_table_fin, file = paste(dir,"/complete_table_fin_", sample, ".rds",sep=""))
