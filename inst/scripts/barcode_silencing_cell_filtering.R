...
print(paste0("starting filtering step, sample ", sample))
...
ValidCell_df <- tmp2 %>%
  ...
  mutate(
    UMI = NULL,
    True_UCIxcellID = ifelse(True_UCIxcellID == 1 & max(UMIxUCI) / max(UMIxUCI[UMIxUCI != max(UMIxUCI)]) >= ratio | 
...
saveRDS(trans, file = paste(dir,"/silencing_matrix_", sample, ".rds",sep=""))
