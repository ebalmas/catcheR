...
#
 ...
 do
     echo $i
     Rscript /home/barcode_silencing_explorative_analysis.R "$D"/ "$ref" "$mode" "$i" "$x" "$y"
     wait
 done
...
do
    ...
    if [ -z "$UMI_count" ]; then
        ...
        filename="/data/scratch/Results_"$i"/UMI_threshold.txt"
        echo $filename
    ...
    Rscript /home/barcode_silencing_cell_filtering.R "$D/" 15 "$UMI_count" "$ratio" "$matrix" "$i"
    wait
done
 ...
 do
     #run epmty selection
     Rscript /home/barcode_silencing_empty_selection.R "$D" "$matrix" "$UMI_count" "$i" "TACGCGTTCATCTGGGGGAGCCG"
     wait
 done

Rscript /home/barcode_silencing_all_samples.R "$D" "$s" 
